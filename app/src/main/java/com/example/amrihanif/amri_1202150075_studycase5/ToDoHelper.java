...
public class ToDoHelper extends SQLiteOpenHelper{
    ...
    public boolean masukkandata(ModelToDo model) {
    ...
    public void getAllItems(ArrayList<ModelToDo> list) {
        Cursor cr = this.getReadableDatabase().rawQuery("SELECT name, description, priority FROM " + TABLE_NAME, null);
        while (cr.moveToNext()) {
            list.add(new ModelToDo(cr.getString(0), cr.getString(1), cr.getString(2)));
        }
    }
}